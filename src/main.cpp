...
void interaction() {
    ...
    while (true) {
        ...
        if (cmd == "ls") {
            ...
            cmd = cmd.substr(7);
            auto filename1 = cmd.substr(0,cmd.find(' '));
            if (filename1 != "") {
                auto filename2 = cmd.substr(cmd.find(" ")+1);
            ...
            size_t pos = cmd.find(' ');
            if (pos != string::npos) {
                auto src = fs.getCurNode()->getName();
                auto dest = cmd.substr(pos + 1);
